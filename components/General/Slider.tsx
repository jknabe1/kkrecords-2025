...
// Updated interface to include custom slide flag
interface SliderProps {
  items: {
    ...
    imageUrl: string
    date?: string
    isSeeMore?: boolean // Flag to identify "See More" slides
  }[]
...
export default function DynamicSlider({ items, itemType }: SliderProps) {
  ...
  // Use the useKeenSlider hook instead of manual initialization
  ...
  const renderItems = () => {
    return items.map((item) => {
      // Render "See More" slide
      if (item.isSeeMore) {
        return (
          <div key={item._id} className={isMobile ? "keen-slider__slide" : ""}>
            <Link
              href={`/${itemType}s`}
              className="relative group overflow-hidden h-full bg-black flex items-center justify-center aspect-[4/5] lg:aspect-[6/5]"
            >
